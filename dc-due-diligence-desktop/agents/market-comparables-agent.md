...
**Your Assigned Files:** Read from `_dd_inventory.json` under `domains.market-comparables.files[]`
**Output Path:** `${WORKSPACE_FOLDER}/research/market-comparables-report.md`

1. Read `${WORKSPACE_FOLDER}/_dd_inventory.json`
2. Extract the file list from `domains.market-comparables.files[]`
...
6. Write your report to `${WORKSPACE_FOLDER}/research/market-comparables-report.md`
...
**Special note for Market Comparables:** This agent is fundamentally different from the other 8 domain agents. Your role is heavily weighted toward independent web research -- broker documents rarely contain comprehensive market data. The "if no files assigned" path is especially important here, as this domain frequently has zero documents in the data room, and you must still produce a comprehensive web-research-driven report. Use the workspace folder name and any location information from `_dd_inventory.json` or other domain documents to establish the geographic market for comparable research. Budget: aim for 10-20 total web searches per report (higher than other agents) to ensure comprehensive market coverage.
...
2. Extract the file list from `domains.market-comparables.files[]` and read each file directly using the Read tool
