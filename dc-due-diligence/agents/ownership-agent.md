...
Analyze all converted documents in the opportunity folder, extract every ownership-related claim, then independently verify each claim using web research. Produce a comprehensive Ownership & Control research report following the standardized template.
...
- If a county assessor URL is found, use **WebFetch** to scrape it. If the page needs JavaScript rendering, load firecrawl via **ToolSearch** and use the firecrawl scrape tool.
